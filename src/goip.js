  ...
  send: function(lineNumber, phoneNumber, textContent, callback) {
    if (goip.isSending) {
      ...
      if (goip.retries < goip.retryMax) {
        goip.dataSend(
          'tools.html?type=sms&line=' + lineNumber,
          'GET',
          null,
          function(htmlData) {
  ...
  getSentStatus: function(lineNumber, callback) {
    if (goip.retries < goip.retryMax) {
      ...
      goip.dataSend(
        ...
        'post',
        { line: lineNumber },
        function(resultCheckData) {
          if (resultCheckData.indexOf('<status' + lineNumber + '>') > -1) {
            let sendResult = resultCheckData
              .split('<status' + lineNumber + '>')[1]
              .split('</error' + lineNumber + '>')[0]
              .replace('</status' + lineNumber + '>', '')
              .replace('<error' + lineNumber + '>', '')
              .replace('\t', '')
              .split('\n')
  ...
  getInbox: function() {
    goip.dataSend('tools.html?type=sms_inbox', 'GET', null, function(pageHTML) {
      ...
      if (jsData) {
        ...
        jsData = jsData
          .substr(0, jsData.indexOf('</script>'))
          .split('sms_row_insert(l')
        jsData.pop()
          ...
          for (let n = 0; n < goip.inbox[i].length; n++) {
            ...
            if (goip.inbox[i][n].length > 0) {
              ...
              if (
                ...
                goip.inbox[i][n] &&
                goip.oldInbox[i][n] &&
                goip.inbox[i][n].join(',').trim() !=
                  goip.oldInbox[i][n].join(',').trim()
              ) {
                var postData = querystring.stringify({
                  to: i,
                  text: goip.inbox[i][n][1],
                  from: goip.inbox[i][n][0],
                  timestamp: goip.inbox[i][n][2]
                })
                if (goip.lineData) {
